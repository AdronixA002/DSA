class Solution59 {
    public int earliestFinishTime(int[] landStartTime, int[] landDuration, int[] waterStartTime, int[] waterDuration) {
        ...
        for(int i = 0 ; i < n2 ; i++){
            if(i<n1 && lTime <= waterStartTime[i]){
                r = Math.min(r, waterStartTime[i] + waterDuration[i]);
            }else if(i<n1 && lTime > waterStartTime[i]){
                r = Math.min(r, lTime + waterDuration[i]);
            }

            if(i<n && wTime <= landStartTime[i]){
                r1 = Math.min(r1, landStartTime[i] + landDuration[i]);
            }else if(i<n && wTime > landStartTime[i]){
                r1 = Math.min(r1, wTime + landDuration[i]);
            }
        }
